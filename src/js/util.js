...
( // Module boilerplate to support commonjs, browser globals and AMD.
  ...
  function createComponent(component, parent, owner) {
    ...
    function apply(top, newTree, oldTree) {
      ...
      for (index = 0, length = newKeys.length; index < length; index++) {
        ...
        // Handle component nodes
          ...
          if (!item) {
            item = oldTree[key] = createComponent(newItem.component, top,
              instance);
            item.append();
            // console.log("created")
          }
    ...
    function emit() {
      if (!owner) throw new Error(
        "Can't emit events from top-level component");
  ...
  function nameNodes(raw) {
    ...
    function processItem(nodes, item) {
      ...
      var i = 1;
      var subType = type == "element" ? tag.name : type == "component" ?
        item[0].name : type;
      var id = type === "element" ? tag.ref : type === "component" ? item.key :
        null;
